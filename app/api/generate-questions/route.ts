...
export async function POST(req: Request) {
  try {
    const { documents, difficulty = "medium", questionCount = 10 }: { 
    ...
    const prompt = `다음 문서들을 분석하여 시험 문제를 JSON 형식으로 생성해주세요:
...
다음 형식으로 JSON만 반환하세요 (다른 설명 없이):
{
  ...
  "essayQuestions": ["서술형 문제1", "서술형 문제2", ...${essayCount}개]
    ...
    console.log("[v0] Questions generated with difficulty:", difficulty, "count:", questionCount)

    return Response.json(data)
  } catch (error) {
    console.error("[v0] Generate questions error:", error)
    return Response.json(
      { error: `문제 생성 중 오류: ${error instanceof Error ? error.message : "알 수 없는 오류"}` },
      { status: 500 },
    )
  }
}
